...
(or a DSP voice method) and runs both through ``VideoAnonymization.anonymize_video``
— the output video has a swapped identity AND an anonymized voice.

This is the union of two single-concern scripts:
  * tests/test_anonymize_video_swap.py   (face swap only)
  * tests/test_anonymize_video_audio.py  (voice only)
...
Usage:
    ...
    python -m tests.test_anonymize_video_full_pipeline --input clip.mp4
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Face swap + voice conversion in one pass "
            "(anonymize_video with SwapOptions + AudioOptions)."
        )
    )
