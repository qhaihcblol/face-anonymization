"""Offline face-swap pipeline using ``VideoAnonymization.anonymize_video``.

Drives the model-based face swap (BlendSwap) via ``visual=SwapOptions(...)``. The
audio track is muxed through untouched unless ``--no-audio`` is passed.

Usage:
    # Swap every face to the bundled source identity, keep the original audio.
    python -m tests.test_anonymize_video_swap --input clip.mp4

    # Pick a source face, trim to the first 5 s, drop the audio.
    python -m tests.test_anonymize_video_swap --input clip.mp4 \
        --source my_face.png --start-sec 0 --end-sec 5 --no-audio
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "VideoAnonymization.anonymize_video with SwapOptions"
        )
    )
...
def main() -> None:
    ...
    result = video_anonymization.anonymize_video(
        ...
        audio=AudioOptions(keep_audio=not args.no_audio),
