"""Anonymize a speaker's voice (no-model DSP or ONNX model path).

Two directions, mirroring the face pipeline:

  * No model (librosa DSP) — mcadams / pitch / formant / pitch_formant. Runs
    anywhere, no weights to download.
  * Model (ONNX voice conversion) — ``--method convert`` (kNN-VC).

The faces are left untouched (``VisualOptions(method="none")``); only the audio is
processed, via ``audio=AudioOptions(anonymize_voice=True, ...)``.

Usage:
    # Quick audition: just process the audio track to a WAV (no video re-encode).
    python tests/test_anonymize_video_audio.py --input clip.mp4 --audio-only \
        --method pitch_formant --pitch-steps -4 --formant-shift 1.2

    # Full video: faces untouched, only the voice is anonymized.
    python tests/test_anonymize_video_audio.py --input clip.mp4 --method pitch

    # Model path (kNN-VC): supply the encoder + vocoder ONNX and a reference voice.
    python tests/test_anonymize_video_audio.py --input clip.mp4 --audio-only \
        --method convert --encoder wavlm.onnx --vocoder hifigan.onnx \
        --reference-voice target.wav
"""
...
def main() -> None:
    ...
    result = pipeline.anonymize_video(
        input_path=args.input,
        output_path=args.output,
        # Faces stay untouched; run the detector sparsely since it does nothing here.
        visual=VisualOptions(method="none", detect_interval=30),
        audio=AudioOptions(
            keep_audio=True,
